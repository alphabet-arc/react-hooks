...
class Home extends Component {
  ...
  async componentDidMount() {
    if (this.getLocalStorage()) {
      ...
      const flatData = await this.handleGetMembers("/api/tracker/members/display");
      if (flatData) {
        const data = this.groupByTeam(flatData, team || []);
        this.setState({ data });
      }
    } else {
  ...
  groupByTeam = (flatData, teams) => {
    const techNames = teams.map((t) => t.name);
    // Also include any tech names in data not in teams
    flatData.forEach((m) => {
      if (!techNames.includes(m.technology_name)) techNames.push(m.technology_name);
    });
    return techNames
      .map((name) => flatData.filter((m) => m.technology_name === name))
