...
class Home extends Component {
  ...
  async componentDidMount() {
    if (this.getLocalStorage()) {
      ...
      if (initialData) {
        const data = this.groupData(initialData);
  ...
  groupData = (flatData) => {
    ...
    const order = [];
    flatData.forEach((m) => {
      if (!map[m.technology_name]) {
        map[m.technology_name] = [];
        order.push(m.technology_name);
      }
      map[m.technology_name].push(m);
    });
    return order.map((tech) => map[tech]);
  };
  ...
  handleGetMembers = async (url) => {
    try {
      ...
      const grouped = this.groupData(flatData);
      this.setState({ data: grouped, initialData: flatData });
      return flatData;
    } catch (e) {
      console.log(e);
    }
