...
class Home extends Component {
  ...
  handleChecked = (value) => {
    this.setState({ checked: value, experienceFilter: "" });
  };
  ...
  handleClear = async () => {
    this.setState({ experienceFilter: "", checked: "Experience" });
    await this.handleGetMembers("/api/tracker/members/display");
  };
  ...
  handleGo = async () => {
    ...
    } else if (checked === "Team" && experienceFilter) {
      url += `?tech=${experienceFilter}`;
  ...
  render() {
    const {
      ...
      experienceFilter, checked,
    } = this.state;

    const isGoEnabled = experienceFilter !== "";
