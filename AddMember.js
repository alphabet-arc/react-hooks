...
class AddMember extends Component {
  ...
  removeTeamRequest = async (name) => {
    try {
      const response = await fetch(`/api/tracker/technologies/remove/${name}`, {
        method: "Delete",
        headers: { Authorization: `Bearer ${this.getLocalStorage()}` },
      });
      return response.status;
        ...
        <form>
          ...
          <div>
            <input type="number" name="empId" placeholder="Employee ID"
              value={empId} onChange={(e) => this.handleChange(e)} />
            <span>{errorStmtEmpId}</span>

            <input type="text" name="empName" placeholder="Employee Name"
              value={empName} onChange={(e) => this.handleChange(e)} />
            <span>{errorStmtEmpName}</span>

            <select name="teamName" value={teamName} onChange={(e) => this.handleChange(e)}>
            ...
            {createTeam && (
              <div className="addList">
                ...
                <input type="text" name="newTeam" value={newTeam}
                  onChange={(e) => this.handleChange(e)} />
                <button type="button" onClick={(e) => this.handleSave(e)}>Save</button>
                ...
                <table>
                  <tbody>
                    {teams.map((t) => (
                      <tr key={t._id}>
                        <td>{t.name}</td>
                        <td><img src={remove} alt="x"
                          onClick={(e) => this.handleRemoveTeam(e, t.name)} /></td>
                      </tr>
                    ))}
                    <tr>
                      <td>
            ...
            <input type="number" name="experience" placeholder="Experience"
              value={experience} onChange={(e) => this.handleChange(e)} />
          ...
          <div>
            <button className="button" type="button"
              ...
              onClick={(e) => this.handleAddMember(e)}>Add</button>
            <button className="button" type="button"
